...
for dataset in delicious-3d_sorted012.tns nell-1_sorted012.tns nell-2_sorted012.tns flickr-3d_sorted012.tns freebase_music_sorted012.tns freebase_sampled_sorted012.tns 1998DARPA_sorted012.tns brainq_fixed_sorted012.tns; do
	...
	log1=`$bin -i $path/$dataset -m 0 -R 32 -w 4 -t 12 | perl -p -e 's/\n//'`
	log2=`$bin -i $path/$dataset -m 0 -R 32 -w 4 -t 12 -f 128 | perl -p -e 's/\n//'`
	log3=`$bin -i $path/$dataset -m 0 -R 32 -w 4 -t 8 | perl -p -e 's/\n//'`
	log4=`$bin -i $path/$dataset -m 1 -R 32 -w 4 -t 8 | perl -p -e 's/\n//'`
	log5=`$bin -i $path/$dataset -m 2 -R 32 -w 4 -t 8 | perl -p -e 's/\n//'`
	log6=`$bin -i $path/$dataset -m 0 -R 32 -w 4 -t 8 -f 128 | perl -p -e 's/\n//'`
	log7=`$bin -i $path/$dataset -m 1 -R 32 -w 4 -t 8 -f 128 | perl -p -e 's/\n//'`
	log8=`$bin -i $path/$dataset -m 2 -R 32 -w 4 -t 8 -f 128 | perl -p -e 's/\n//'`
