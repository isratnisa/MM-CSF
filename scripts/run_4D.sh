...
for dataset in uber_sorted0123.tns nips_sorted0123.tns chicago-crime-comm_sorted0123.tns flickr-4d_sorted0123.tns; do
	...
	log6=`$bin -i $path/$dataset -m 0 -R 32 -w 4 -t 8 | perl -p -e 's/\n//'`
	log7=`$bin -i $path/$dataset -m 0 -R 32 -w 4 -t 8 -f 128 | perl -p -e 's/\n//'`
	log8=`$bin -i $path/$dataset -m 0 -R 32 -w 4 -t 8 -f 128 -l 2| perl -p -e 's/\n//'`
	log9=`$bin -i $path/$dataset -m 0 -R 32 -w 4 -t 8 -l 2 | perl -p -e 's/\n//'`
